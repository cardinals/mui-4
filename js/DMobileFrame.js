	...
	define('scripts/Core/ImageUtil.js', function(exports) {
		...
		(function(ImgLoaderFactory) {
			...
			var defaultSettingOptions = {
				...
				//默认图片的基座路径
				//'defaultImgBase': 'img/mobile/',
				//loading图片的名称
				//'loadingImgName': 'loading.png',
				//error图片名称
				//'errorImgName': 'load_falid.png',
				//图片缓存的时间戳,毫秒单位,默认为1天
				'imgsTimeStamp': 1000 * 60 * 60 * 24 * 100,
...
//				}
			...
			/**
			 * @description 所有图片缓存的大小
			 * @param {Function} successCallback 成功回调
			 * @param {Function} errorCallback 失败回调
			 */
			ImgLoaderFactory.loaderImgsCacheSize = function(successCallback, errorCallback) {
				MobileFrame.plusReady(function() {
					//遍历目录文件夹下的所有文件，然后删除
					var tmpUrl = plus.io.convertLocalFileSystemURL(defaultSettingOptions['imgStoragePath']);
					//需要手动加上 file://
			...
			function setImgFromNet($img, loadUrl, relativePath) {
				...
				debug && console.log("网络下载："+loadUrl);
				//下载参数
				var options = {
