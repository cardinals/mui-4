...
function lazyload(obj, callback){
    ...
    temp_img.onload = function(){
        debug && console.log('存在本地缓存图片文件'+absolute_image_path+'，直接显示');
                ...
                if(status != 200){
                    ...
                    if(local_image_url != null){
                        plus.io.resolveLocalFileSystemURL(local_image_url, function(entry) {
                            ...
                            debug && console.log("获取文件失败：" + JSON.stringify(error));
                        });
                    }
                }else{
                    // 把下载成功的图片显示
 ...
 * 图片缓存的图片大小
 */
function lasyloadSize(fun_c){
    var local_image_path     = '_downloads/image'; // 缓存本地图片url
    var absolute_image_path = plus.io.convertLocalFileSystemURL(local_image_path); // 平台绝对路径

    //console.log("--1absolute_image_path:"+absolute_image_path);
    ...
    plus.io.resolveLocalFileSystemURL(absolute_image_path, function(entry) {
        ...
        console.log("1.1error:"+JSON.stringify(error));
        fun_c(0);
    });
}
 ...
 * 清空目录缓存
 */
function lasyloadClear(fun_c){
    var local_image_path = '_downloads/image'; // 缓存本地图片url
    var absolute_image_path = plus.io.convertLocalFileSystemURL(local_image_path); // 平台绝对路径

    console.log("--2absolute_image_path:"+absolute_image_path);
